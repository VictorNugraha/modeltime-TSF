...
<i>Disclaimer</i>, Idealnya ketika kita membuat model <i>time series forecasting</i>, model tersebut harus lulus dari setiap uji asumsi yang dilakukan tanpa adanya pengecualian. Jika kita tetap memaksakan untuk menggunakan model yang tidak lulus uji asumsi, kita tidak bisa mempercayai keakuratan model tersebut dalam memprediksi walaupun sudah ada nilai tolerasi dari evaluasi error.
...
Berdasarkan keputusan yang bisa diambil oleh pemerintah Amerika Serikat dari hasil prediksi, kita harus bisa membuat sebuah model yang dapat dipercaya. Untuk mendapatkan model yang dapat dipercaya, kita bisa melakukan penyetelan manual terhadap <i>hyperparameter</i> setiap model yang kita gunakan atau kita bisa mencoba beberapa model lainnya seperti ARIMA Boost, prophet Boost atau Random Forest.
